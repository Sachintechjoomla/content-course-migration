 ...
 * - Processing CSV files containing content metadata
 * - Downloading and processing various file types (PDF, MP4, ZIP, etc.)
 * - Uploading files to AWS S3 with streaming support
 * - Creating content nodes in Sunbird platform
 * - Handling content review and publishing workflows
 * - Managing content thumbnails and icons
 *
 * @example
 * ```typescript
 * // Process a single content record
 * const result = await contentService.processSingleContentRecord(contentRecord);
 *
 * // Upload an icon for content
 * const iconResult = await contentService.uploadIcon(doId, iconUrl, userToken);
 * ```
 */
@Injectable()
export class ContentService {
  ...
  constructor(
    ...
    this.logFilePath = path.join(process.cwd(), "error.log");
  }

  /**
   * Processes a single content record through the complete workflow.
   *
   * This method handles the entire content creation pipeline:
   * 1. Validates the content record
   * 2. Creates content node in Sunbird
   * 3. Downloads and processes the main file
   * 4. Uploads file to S3
   * 5. Uploads content to Sunbird
   * 6. Reviews and publishes the content
   * 7. Handles thumbnail upload if present
   *
   * @param record - The content entity object containing metadata and file URLs
   * @returns Promise<string | undefined | false> - Returns the content ID (doId) on success, false on validation failure, or undefined on error
   *
   * @example
   * ```typescript
   * const contentRecord = {
   *   cont_title: "Sample Content",
   *   cont_dwurl: "https://example.com/file.pdf",
   *   cont_thumb: "https://example.com/thumbnail.jpg"
   * };
   *
   * const result = await this.processSingleContentRecord(contentRecord);
   * if (result) {
   *   console.log(`Content created with ID: ${result}`);
   * }
   * ```
   */
  ...
  */
  ...
  async uploadIcon(
    ...
    try {
      ...
      // --- Step 1: Handle Google Drive shared links ---
      if (iconUrl.includes("drive.google.com")) {
        const match = iconUrl.match(/\/d\/([a-zA-Z0-9_-]+)/);
        if (match && match[1]) {
          const fileId = match[1];
          iconUrl = `https://drive.google.com/uc?export=download&id=${fileId}`;
        }
      }

      // --- Step 2: Download file into memory ---
      ...
      let fileName = path.basename(iconUrl.split("?")[0]);
      if (!fileName || !fileName.includes(".")) {
        fileName = uuidv4(); // fallback unique name
      }
  ...
  async updateRecordsChangeCreatedBy(limit = 500): Promise<any> {
    try {
      const userToken = this.configService.get<string>("USER_TOKEN") || "";
      const userId = this.configService.get<string>("USER_ID") || "";

      // const fetched = await this.getPublishedContents(limit);

      // if (fetched.count == 0 || fetched.content.length === 0) {
      //   console.log("No published contents found");
      //   return { error: "No published contents found" };
      // }

      const doIds = ["do_214358500233756672128"];

      // for (const record of fetched.content) {
      for (const doId of doIds) {
        try {
          // (Optional) ensure freshest data; skip if you already trust record.createdBy
          const read = await this.fetchContent(
            doId,
            this.configService.get<string>("USER_TOKEN") || ""
          );
          ...
          const payload = {
            request: {
              content: {
                versionKey: read.versionKey, // required
                primaryCategory: "Activity", // from fetch
                // lastUpdatedBy: userId, // from logged in user
                // subject: record.subject, // example override
                // contentType: record.contentType || "Resource",
                // name: record.name, // example override
                // program: record.program
                //   ? record.program
                //   : ["Early Childhood Education"], // example override
                // subDomain: record.subDomain
                //   ? record.subDomain
                //   : ["Growth & Learning"], // example override
                // targetAgeGroup: record.targetAgeGroup
                //   ? record.targetAgeGroup
                //   : ["3-6 yrs"], // example override
                // primaryUser: record.primaryUser
                //   ? record.primaryUser
                //   : ["Educators", "Learners/Children"], // example override
                // contentLanguage: record.contentLanguage
                //   ? record.contentLanguage
                //   : record.language, // example override
                // mimeType: record.mimeType, // example override
              },
            },
          };
          ...
          const response = await this.retryRequest(
              ...
              axios.patch(
                ...
                payload,
                {
                  headers: {
                    ...
                    "user-id": userId,
                    tenantId:
                      this.configService.get<string>("MIDDLEWARE_TENANT_ID") ||
                      "",
                    Authorization: `Bearer ${userToken}`,
                  },
                }
            ...
            `update content ${doId}` // log/context message
          );

          if (!response.data || !response.data.result) {
            console.error(
              `Update failed or no result for ${doId}:`,
              response.data
            );
          ...
          // Step 3: Review Content
          const reviewedContent = await this.reviewContent(doId, userToken);
          console.log("Reviewed Content:", reviewedContent);

          if (!reviewedContent) {
            console.error(`Review failed for ${doId}`);
            continue; // Skip to next record
          }
          // Step 4: Publish Content
          const publishedContent = await this.publishContent(doId, userToken);

          console.log("Update publishedContent:", publishedContent);
          // return response.data;
        } catch (err) {
          console.error(`Error processing ${doId}`, err);
        }
      }
      ...
      console.log("Params:", error.response?.data?.params);
      return { error: "Failed to fetch published contents" };
    }
  }

  async fetchContent(doId: string, userToken: string) {
  ...
  async getPublishedContents(limit = 1): Promise<any> {
    try {
      const userToken = this.configService.get("USER_TOKEN"); // Replace with dynamic token if needed
      const headers = {
        Authorization: `Bearer ${userToken}`,
        "Content-Type": "application/json",
        Accept: "application/json, text/plain, */*",
      ...
      const requestBody = {
        request: {
          filters: {
            ...
            // query: "do_21436331797150105611509",
            createdBy: {
              "!=": this.configService.get("USER_ID"),
            },
          },
      ...
      this.logger.log(`📦 Fetching published contents: ${url}`);
