...
export class CourseService {
  ...
  async importCourses(limit = 51) {
    ...
    for (const tableName of courseTables) {
      ...
      `);
        ...
        SELECT id, course_title, course_do_id, status, course_metadata
        FROM ${tableName}
        ...
        for (const course of courses) {
          try {
            ...
            if (!course.course_do_id) {
              ...
              const existingCoursePresent = await this.findCourseByTitle(course);

              if (existingCoursePresent.count) {
                course_do_id = existingCoursePresent.indentifier;
                // const updateCourse = await this.updateCourse(
                //   existingCoursePresent.indentifier,
                //   course,
                //   frameworkData
                // );
  ...
  async createCourse(course: any, frameworkData: any): Promise<string> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        await this.createSets(course_do_id, course, frameworkData);
  ...
  async createSets(course_do_id: string, course: any, frameworkData: any) {
    console.log(
      "Creating Level 1 sets for course:",
      JSON.stringify(course, null, 2)
    );

    // Fetch course metadata and sets_do_id both
    const courseData = await this.courseRepository.query(
      `
        SELECT sets_do_id, course_metadata FROM ${this.courseSetsTable} WHERE course_title = ?
    `,
      [course.course_title]
    );
    ...
    */
    ...
    await this.courseRepository.query(
      `
      UPDATE ${this.courseSetsTable} SET sets_do_id = ? WHERE course_title = ?
  `,
      [setsJson, course.course_title]
    );
    ...
    if (hasChildren) {
      ...
      await this.createSubSets(course_do_id, sets, course, resolvedIds);
    } else {
  ...
  async createSubSets(
    ...
    console.log("💾 Updated nested sets:", setsJson);
  ...
  async createSubSetsBatchAPI(
    ...
    const courseMetaResult = await this.courseRepository.query(
      `SELECT course_metadata FROM ${this.courseSetsTable} WHERE course_title = ?`,
      [course.course_title]
    );
  ...
  async taxonomyMappingVocationalTrainingCourses(limit = 1) {
    ...
    console.log(notInCourseList);
    for (const course_do_id of notInCourseList) {
      ...
      const requestBody = {
        request: {
          filters: {
            ...
            program: [courseMetadata.program || "Open School"],
